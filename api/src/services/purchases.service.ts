  ...
  try {
    const result = await sequelize.transaction(async (transaction) => {
      ...
      if (isExpired) {
        ...
        let restored: Array<{ id: string; available_stock: number }> = [];
        if (expiredRows.length) {
          restored = (await sequelize.query(
            `
              UPDATE drops
        ...
        return {
          ...
          effects:
            expiredRows.length && restored[0]
              ? {
                  reservationExpired: { dropId: params.dropId, reservationId: reservation.id },
                  stockUpdated: { dropId: params.dropId, availableStock: restored[0].available_stock }
                }
              : null
        };
      }
    ...
    if (!result.ok) {
      if ((result as any).effects?.stockUpdated) {
        emitStockUpdated((result as any).effects.stockUpdated);
      }
      if ((result as any).effects?.reservationExpired) {
        emitReservationExpired((result as any).effects.reservationExpired);
      }
